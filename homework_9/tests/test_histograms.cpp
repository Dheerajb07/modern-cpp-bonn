...
TEST(Histogram, CreateFromDictionary) {
    ...
    ASSERT_EQ(histogram.size(), dictionary.size())
            << "The numbers of bins in the histogram must match the number of "
               "words "
               "in the dictionary";
...
TEST(Histogram, Iterators) {
    ...
    ASSERT_EQ(sum, descriptors.rows)
            << "The number of the input descriptors must match the sum of all "
               "the "
...
TEST(Histogram, NonTrivialExample) {
    ...
    // we have:
    //  05,  05,  05,  05,  05,  05,  05,  05,  05 <--- closest cluster == 1
    //  (00) 15,  15,  15,  15,  15,  15,  15,  15,  15 <--- closest cluster ==
    //  2 (20)
    // 115, 115, 115, 115, 115, 115, 115, 115, 115 <--- closest cluster == 5
    // (80) Therefore, the histogram must look like: bins [1, 1, 0, 0, 1]
