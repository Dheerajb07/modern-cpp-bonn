...
cv::Mat get_random_centroids(const cv::Mat &points,
                             const int k,
    ...
    for (int cluster_idx = 0; cluster_idx < k; cluster_idx++) {
        ...
        } while (rand_idx.find(point_idx) !=
                 rand_idx.end());  // Ensure it's unique
...
void clusterPoints(const cv::Mat &points,
...
float totalWCSS(const std::vector<cv::Mat> &cluster, const cv::Mat &centroids) {
    ...
    for (int cluster_idx = 0; cluster_idx < k; cluster_idx++) {
        ...
        float wcss = std::inner_product(dist.begin(), dist.end(), dist.begin(),
                                        0.0f);
