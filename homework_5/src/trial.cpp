...
int main(){
    ...
    // serialize img
    ipb::serialization::Serialize(img,std::filesystem::current_path() / "data/lenna.bin");
    // std::cout << "Serialized image" << std::endl;

    // deserialize image
    cv::Mat deserial_img = ipb::serialization::Deserialize(std::filesystem::current_path() / "data/lenna.bin");
